...
class FF1_SM4(FF1):
    def __init__(self, key, tweak=None, radix=10, ):
        ...
        self.sm4 = SM4Encryption(key)

    def round_function(self, padding, q, d):
        """ 计算--S """
        # 计算--R=PRF(P || Q)，返回的结果为字节字符串bytes类型--b'\x01\x02\x03'
        r = self.sm4.encrypt(padding + q) # 此处包括其他地方，最终需要确认变量的类型，字符串/字节
...
if __name__ == '__main__':
    cipher = FF1_SM4.withCustomAlphabet(b'0123456789abcdef', None, string.digits + string.ascii_lowercase + string.ascii_uppercase)
    # message = "90313454390"
    # message = "sxdfrgfdgfb"
